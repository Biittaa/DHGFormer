...
def main(args, config, fold_idx, kfold, base_seed):
    ...
    # NOTE: this optimizer construction is now identical to main.py's. The
    # previous version here used plain `torch.optim.Adam(model.parameters(),
    # weight_decay=...)`, which applied weight_decay to
    # temporal_encoder.gate/pos_embed too. main.py deliberately excludes those
    # from weight decay so the gate (initialized near 0 via sigmoid(-3)) is
    # free to open up during training without decay constantly pulling it
    # back down. Keeping the two optimizers different was a likely cause of
    # kfold runs and single-split runs (main.py) behaving differently when
    # both temporal_transformer and smri were enabled together.
    no_decay_params = []
    decay_params = []
