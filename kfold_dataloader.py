...
from imports.smri_graph_build import (
    build_view_node_features,
    build_fold_graphs,
...
def load_smri_features(dataset_config, num_subjects):
    """Used only for smri_encoder_type in {'fcn', 'transformer'}.
    For 'multiview_gcn' the per-view features come from build_view_node_features
    instead (see init_dataloader_kfold)."""
...
def _load_raw_tensors(dataset_config):
    """Loads/preprocesses everything EXCEPT splitting into train/val/test.
    Kept in lockstep with dataloader.py's init_dataloader preprocessing.
    Now also carries 'site' (from abide.npy) so the multiview_gcn branch below
    can call build_view_node_features exactly like dataloader.py does."""
    ...
    # For 'multiview_gcn' the real per-view features are built inside
    # init_dataloader_kfold (they need the fold-specific train_idx to fit
    # ComBat / build the kNN graph without leakage) — placeholder here.
...
def get_kfold_split_indices(labels_np, kfold, fold_idx, val_ratio=0.1, seed=123):
    """
    Stage 1: Stratified K-Fold -> Test = 1/kfold of total data
    Stage 2: Stratified split of remaining data -> Validation = val_ratio of
             Train+Validation, Train = remaining samples
    """
    labels_flat = np.asarray(labels_np).reshape(-1)
    ...
    outer_skf = StratifiedKFold(n_splits=kfold, shuffle=True, random_state=seed)
    splits = list(outer_skf.split(np.zeros(len(labels_flat)), labels_flat))
    ...
    train_relative_idx, val_relative_idx = next(
        inner_splitter.split(np.zeros(len(train_val_idx)), train_val_labels)
    )
...
def init_dataloader_kfold(dataset_config, fold_idx, kfold=5, val_ratio=0.1, seed=123):
    """Real stratified k-fold cross validation loader. Call this once per
    fold_idx (0..kfold-1) from kfold_main.py. Kept in lockstep with
    dataloader.py's init_dataloader (single-split) preprocessing."""
    ...
    if use_smri and smri_encoder_type == "multiview_gcn":
        ...
        # train_idx/labels/site now passed through (previously missing) so
        # ComBat / Ridge-RFE / any train-only fitting inside
        # build_view_node_features only ever sees this fold's train subjects
        # -- matches dataloader.py's init_dataloader exactly.
        view_node_names, view_node_features = build_view_node_features(
            dataset_config, num_subjects,
            labels=raw["labels_np"], train_idx=train_idx, site=raw["site"]
        )
        ...
        # extra features (use_smri_global / use_pheno) were previously never
        # built in the kfold path -- added to match dataloader.py.
        extra, _ = build_extra_features(dataset_config, num_subjects, train_idx=train_idx)
        extra_dim = 0
        if extra is not None:
            smri_flat = np.concatenate([smri_flat, extra], axis=1)
            extra_dim = extra.shape[1]

        smri_features_t = torch.from_numpy(smri_flat).float()
        smri_dim = smri_flat.shape[1]

        k_per_view = dataset_config.get(
            "mvgcn_k_neighbors", {"aseg": 8, "aparc": 32, "wmparc": 16}
        )
        # metric_per_view was previously not forwarded, silently forcing
        # 'pearson' for every view regardless of the yaml setting.
